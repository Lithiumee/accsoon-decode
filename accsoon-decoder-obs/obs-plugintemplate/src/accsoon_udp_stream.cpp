...
void udpReceive(int sock)
{
    ...
    uint32_t gst_first_flag = 1;
    // appsrc has set is-live and do-timestamp to true, no need to set timestamp
    // GstClockTime timestamp = 0;
    // GstClockTime init_timestamp = 0;
    // uint64_t last_time = os_gettime_ns();
    uint64_t duration = gst_util_uint64_scale_int(1, GST_SECOND, 30);// we still need to set duration to get interval between frames right
            ...
            if (message.frags.size() == message.frac_cnt)
            {
                ...
                if (gst_first_flag)
                {
                    ...
                    else
                    {
                        ...
                        gst_first_flag = 0;
                        // init_timestamp = gst_element_get_base_time(appsrc);
                    }
                }
                ...
                gst_buffer = gst_buffer_new_allocate(NULL, len, NULL);
                if (gst_buffer == NULL)
                {
                    LOG(LOG_ERROR, "Failed to allocate gst_buffer");
                    continue;
                }
                // reconstruct the frame
                latest_pointer = 0;
                gst_buffer_fill(gst_buffer, latest_pointer, message.frags[0].data() + MSG_HEADER_SIZE, message.frags[0].size() - MSG_HEADER_SIZE);
                latest_pointer += message.frags[0].size() - MSG_HEADER_SIZE;
                for (uint16_t i = 1; i < frac_cnt; i++)
                {
                    gst_buffer_fill(gst_buffer, latest_pointer, message.frags[i].data(), message.frags[i].size());
                    latest_pointer += message.frags[i].size();
                }
                // timestamp = gst_clock_get_time(gst_system_clock_obtain());
                // GST_BUFFER_PTS(gst_buffer) = timestamp - init_timestamp;
                // GST_BUFFER_DTS(gst_buffer) = timestamp - init_timestamp;
                GST_BUFFER_DURATION(gst_buffer) = duration;

                g_signal_emit_by_name(appsrc, "push-buffer", gst_buffer,
                                      &ret);
